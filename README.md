...
- **Wake word "tarquin"** (hands-free, on-device): a microWakeWord streaming model trained here
  (`tools/wakeword/`) runs on the ESP32-S3 via tflite-micro, ~1 % CPU, no network. Falls back to shipping
  short clips for server-side matching if no model is compiled in.
...
| `tools/wakeword/` | train the "tarquin" wake-word model and embed it (Python, offline); see [docs/wakeword.md](docs/wakeword.md) |
